...
def setup_shared_ui(show_menu=False, menu_options=None):
    ...
    # --- SMART POLLING SETUP ΠΡΟΣΩΡΙΝΑ ΑΠΕΝΕΡΓΟΠΟΙΗΜΕΝΟ ---
    # Υπήρχαν συνεχόμενα reruns στη σελίδα Gantt.
    # Κρατάμε την εφαρμογή πλήρως λειτουργική και αφήνουμε μόνο τη χειροκίνητη
    # "Άμεση Ανανέωση" από το sidebar μέχρι να σταθεροποιηθεί ξανά το auto polling.
    polling_js = ""
    ...
    if utils.supabase:
        ...
        if st.sidebar.button("🔄 Άμεση Ανανέωση", use_container_width=True):
            ...
            try:
                ...
                st.warning(f"Δεν ολοκληρώθηκε η ανανέωση: {e}")
            st.rerun()
    else:
