        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Εκτέλεση Εισαγωγής", type="primary"):
                    ...
                    # Κανονικοποίηση ονομάτων στηλών (μικρά γράμματα, χωρίς κενά, τελείες κλπ)
                    norm_cols = [str(c).lower().strip().replace(' ', '').replace('.', '').replace('_', '') for c in df_import.columns]
                    
                    # Αναζήτηση στήλης Ονόματος
                    name_col = next((orig for orig, c in zip(df_import.columns, norm_cols) if c in ['ονοματεπωνυμο', 'όνομα', 'ονομα', 'name', 'επιθετο', 'επώνυμο', 'ονοματεπώνυμο']), None)
