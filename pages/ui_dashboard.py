...
def render_top_nav(go_prev_week, go_next_week, go_to_today):
    """Εμφανίζει τα κουμπιά πλοήγησης, το ημερολόγιο και τα φίλτρα στο πάνω μέρος."""
...
def render_export_section(export_data, start_of_week):
    """Εμφανίζει το κουμπί εξαγωγής και τις συμβουλές κάτω από το γράφημα."""
...
def render_quick_add_form(selected_date, active_employee_ids):
    ...
    with st.form("quick_add", clear_on_submit=True):
        ...
        if submitted:
            return {
                "add_date": add_date,
                "proj_choice": proj_choice,
                "custom_proj_name": custom_proj_name,
...
def render_edit_form(wk_groups, clicked_key, active_employee_ids):
    """Φόρμα και κουμπιά για επεξεργασία και γρήγορη μετακίνηση βάρδιας."""
