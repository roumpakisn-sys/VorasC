                ...
                if my_eid in emp_day_assigns:
                    ...
                    for pa in emp_day_assigns[my_eid]:
                        if pa.get('id') != a['id']:
                            ...
                            # 1. Είναι διαφορετικό έργο (για να μην βγάζει το ίδιο όνομα ξανά)
                            is_diff_project = (pa_name != a_name)
                            
                            # 2. Η προηγούμενη βάρδια ξεκινάει νωρίτερα ή την ίδια ώρα
                            starts_earlier_or_same = (t_pa_start_str <= t_a_start_str)
                            
                            # 3. Επικάλυψη: Η προηγούμενη ΔΕΝ έχει τελειώσει πριν ξεκινήσει η δεύτερη
                            overlaps = (t_pa_end_str > t_a_start_str)
                            
                            # 4. Η δεύτερη βάρδια τελειώνει ΑΥΣΤΗΡΑ αργότερα από την πρώτη
