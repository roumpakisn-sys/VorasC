...
# --- ΣΤΑΘΕΡΟΠΟΙΗΣΗ CLICK/SCROLL ΓΙΑ ΕΠΕΞΕΡΓΑΣΙΑ ΜΠΑΡΑΣ ---
if "gantt_selected_key" not in st.session_state:
    st.session_state.gantt_selected_key = None
if "gantt_trigger_scroll" not in st.session_state:
    st.session_state.gantt_trigger_scroll = False
if "ignore_next_plotly_event" not in st.session_state:
    st.session_state.ignore_next_plotly_event = False

def clear_bar_selection():
    st.session_state.gantt_selected_key = None
    st.session_state.gantt_trigger_scroll = False
    st.session_state.ignore_next_plotly_event = True
...
if clicked_key:
    if st.session_state.get("ignore_next_plotly_event", False):
        st.session_state.ignore_next_plotly_event = False
    else:
        st.session_state.gantt_selected_key = clicked_key
        st.session_state.gantt_trigger_scroll = True
        st.session_state.ignore_next_plotly_event = True
        st.rerun()

# Όσο είναι ανοιχτό editing state, παγώνει το auto-refresh για να μη "πετάει" τον χρήστη
if st.session_state.get("gantt_selected_key"):
    ...
    if is_full_admin:
                        ...
                        for day_offset in range(duration_days):
                            ...
                            for va in valid_assignments:
                                if va['msg'] == "Allowed Overlap":
                                    st.toast(f"ℹ️ Επιτράπηκε επικάλυψη ωραρίου για τον/την {va['emp_name']} στις {current_assign_date.strftime('%d/%m')}.", icon="ℹ️")
        ...
        with col_edit:
            ...
            if not wk_groups:
                ...
                default_idx = 0
                if st.session_state.get("gantt_selected_key") and st.session_state.get("gantt_selected_key") in group_keys:
                    default_idx = group_keys.index(st.session_state.get("gantt_selected_key")) + 1

                # Scroll ΜΟΝΟ μία φορά μετά από click πάνω στη μπάρα
                if st.session_state.get("gantt_trigger_scroll"):
                    components.html(
                        """
                        <script>
                            ...
                            setTimeout(function() {
                                ...
                                for (const h of headers) {
                                    if (h.innerText && h.innerText.includes('Επεξεργασία Μπάρας')) {
                ...
                if selected_key == "":
                    ...
                    st.session_state.gantt_selected_key = selected_key
                        ...
                        for a_id in target_group['AssignmentIds']:
                            ...
                            if new_a['employeeId']:
                                ...
                                adj_start, adj_end, is_conflict, msg = scheduling.check_and_resolve_conflict(
                                    new_a['employeeId'], new_a['startTime'], new_a['endTime'], day_assigns, exclude_ids=target_group['AssignmentIds']
                                )
                                if is_conflict:
