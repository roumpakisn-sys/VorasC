    ...
    for i in range(7):
        ...
        for l in st.session_state.leaves:
            if l['startDate'] <= curr_date <= l['endDate']:
                emp_full = utils.get_employee_name(l['employeeld'])
        ...
        if not day_assignments:
            ...
            for a in day_assignments:
                ...
                if not a.get('employeeld'):
                    formatted_name = "ΧΩΡΙΣ ΠΡΟΣΩΠΙΚΟ"
                else:
                    full_name = utils.get_employee_name(a['employeeld'])
                ...
                groups[key]['Employeelds'].append(a['employeeld'])
                    ...
                    if str_start >= str_end:
                        ...
                        if errors:
                            ...
                            for va in valid_assignments:
                                ...
                                new_assign = {
                                    'id': str(uuid.uuid4()), 'employeeld': va['eid'], 'projectId': final_proj_id,
                ...
                if selected_key != "":
                    ...
                    if any([move_m_day, move_p_day, move_m_hour, move_p_hour]):
                        ...
                        for a_id in target_group['AssignmentIds']:
                            ...
                            if new_a['employeeld']:
                                emp_name = utils.get_employee_name(new_a['employeeld'])
                                if utils.is_on_leave(new_a['employeeld'], new_a['date']):
                                    st.toast(f"Αδύνατη μετακίνηση: Ο/Η {emp_name} έχει άδεια!", icon="❌")
                                    has_error = True; break
                                adj_start, adj_end, is_conflict, msg = utils.check_and_resolve_conflict(new_a['employeeld'], new_a['date'], new_a['startTime'], new_a['endTime'], exclude_ids=target_group['AssignmentIds'])
                                if is_conflict:
                    ...
                    with st.form("quick_edit"):
                        ...
                        valid_emp_ids = [eid for eid in target_group['Employeelds'] if eid]
                            ...
                            if str_start >= str_end:
                                ...
                                if errors:
                                    ...
                                    for va in valid_assignments:
                                        ...
                                        new_a = {
                                            'id': str(uuid.uuid4()), 'employeeld': va['eid'], 'projectId': final_edit_proj_id,
