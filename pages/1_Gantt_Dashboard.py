...
import scheduling
import gantt_engine
from html_gantt_component import render_gantt_html
...
def get_cached_data(
    ...
    fig, wk_groups, export_data = gantt_engine.generate_gantt_chart(
        start_of_week,
        zoom_factor,
        presentation_mode,
        data_version,
...
if (
    st.session_state.get('last_gantt_params') == current_gantt_params
    and 'cached_fig' in st.session_state
    and not st.session_state.get('data_dirty', False)
):
...
# --- HTML PLOTLY GANTT ΕΜΦΑΝΙΣΗ & EVENT HANDLING ---
clicked_key = render_gantt_html(
    fig,
    height=650,
    key=f"html_gantt_{start_of_week}_{st.session_state.get('local_gantt_version', 0)}"
)
                        ...
                        if custom_proj_name.strip():
                            ...
                            existing_p = next(
                                (
                                    ...
                                    if p['name'].strip().lower() == c_name.lower()
                                ),
                                None
                            )
                            ...
                            if existing_p:
                                ...
                                new_p = {
                                    'id': final_proj_id,
                                    'name': c_name,
                                    'color': config.BASIC_COLORS[color_choice]
                                }
                        ...
                        for day_offset in range(duration_days):
                            ...
                            for eid in emps_to_process:
                                if eid:
                                    ...
                                    if scheduling.is_on_leave(
                                        eid,
                                        current_assign_date,
                                        st.session_state.leaves_by_emp
                                    ):
            ...
            if not wk_groups:
                ...
                if selected_key != "":
                    ...
                    if any([move_m_day, move_p_day, move_m_hour, move_p_hour]):
                        ...
                        for a_id in target_group['AssignmentIds']:
                            ...
                            if delta_hours != 0:
                                ...
                                s_dt = datetime.combine(
                                    dummy_date,
                                    datetime.strptime(str(orig_a['startTime'])[:5], "%H:%M").time()
                                )

                                e_dt = datetime.combine(
                                    dummy_date,
                                    datetime.strptime(str(orig_a['endTime'])[:5], "%H:%M").time()
                                )
                                ...
                                if orig_a.get('arrivalTime'):
                                    arr_dt = datetime.combine(
                                        dummy_date,
                                        datetime.strptime(str(orig_a['arrivalTime'])[:5], "%H:%M").time()
                                    )
                        ...
                        if not has_error:
                            ...
                            st.session_state.assignments = [
                                ...
                                if a['id'] not in target_group['AssignmentIds']
                            ]
                    ...
                    with st.form("quick_edit"):
                        ...
                        default_proj_idx = (
                            proj_ids.index(target_group['ProjectId'])
                            if target_group['ProjectId'] in proj_ids
                            else 0
                        )
                        ...
                        with e_arr:
                            use_arr_edit = st.checkbox(
                                "Με Προσέλευση",
                                value=bool(existing_arr),
                                key="edit_use_arr"
                            )
                            ...
                            new_t_arrival = st.time_input(
                                "Ώρα Προσ.",
                                value=def_arr,
                                key="edit_arrival_time"
                            )
                        ...
                        with c_canc1:
                            e_is_cancelled = st.checkbox(
                                "Επισήμανση ως Ακυρωμένη",
                                value=target_group.get('is_cancelled', False)
                            )

                        with c_canc2:
                            e_cancel_reason = st.text_input(
                                "Λόγος Ακύρωσης (Συμπληρώστε αν ακυρώνετε)",
                        ...
                        if del_edit:
                            old_assigns = [
                                a for a in st.session_state.assignments
                                if a['id'] in target_group['AssignmentIds']
                            ]
                            ...
                            if str_start >= str_end:
                                ...
                                for eid in emps_to_process:
                                    if eid:
                                        ...
                                        if scheduling.is_on_leave(
                                            ...
                                            st.toast(
                                                f"Ο/Η {emp_name} έχει άδεια. Καταχωρήθηκε ως 'Χωρίς Προσωπικό'.",
                                                icon="⚠️"
                                            )
                                        else:
                                            ...
                                            if is_conflict:
                                                ...
                                                st.toast(
                                                    f"Διπλοκράτηση {emp_name}. Καταχωρήθηκε ως 'Χωρίς Προσωπικό'.",
                                                    icon="⚠️"
                                                )
                                            else:
                                                valid_assignments.append({
                                ...
                                if edit_custom_proj_name.strip():
                                    ...
                                    if existing_p:
                                        ...
                                        new_p = {
                                            'id': final_edit_proj_id,
                                            'name': c_name,
                                            'color': config.BASIC_COLORS[edit_color]
                                        }
                                        st.session_state.projects.append(new_p)
                                        utils.db_insert('projects', new_p, track=False)
                                else:
                                ...
                                for va in valid_assignments:
                                    if va['msg'] == "Allowed Overlap":
                                        st.toast(
                                            f"ℹ️ Επιτράπηκε επικάλυψη ωραρίου: {va['emp_name']} ({va['start']})",
                                            icon="ℹ️"
                                        )
