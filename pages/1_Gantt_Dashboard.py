...
import streamlit.components.v1 as components  # <-- ΠΡΟΣΘΗΚΗ: Για το αυτόματο κατέβασμα (scroll)
...
def build_html_gantt(wk_groups, start_of_week, zoom_factor):
    ...
    for i in range(7):
        ...
        for g, lane_idx in group_lanes:
            ...
            # 👇 --- Η ΜΕΓΑΛΗ ΑΛΛΑΓΗ ΠΟΥ ΕΛΥΣΕ ΤΟ ΠΡΟΒΛΗΜΑ ΤΟΥ ΚΛΙΚ --- 👇
            # Αφαιρούμε τα σύμβολα '=' από το τέλος, γιατί μπλοκάρουν την εντολή κλικ!
            safe_key = base64.urlsafe_b64encode(g['Key'].encode('utf-8')).decode('utf-8').rstrip('=')
            
            # Αλλάξαμε το href σε '#' ώστε να το αναγνωρίζει η βιβλιοθήκη st-click-detector
            html += f"<a href='#' id='{safe_key}' draggable='false' class='mygantt-bar' onclick='if(window.gIsDragging){{ event.preventDefault(); event.stopPropagation(); return false; }}' style='position: absolute; left: {left_pct}%; width: {width_pct}%; top: {top_px}px; background-color: {bg_color}; height: 38px; border: 1px solid rgba(0,0,0,0.5); border-radius: 6px; box-shadow: 0 3px 6px rgba(0,0,0,0.15); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: black; text-decoration: none; cursor: pointer; transition: all 0.1s; box-sizing: border-box; overflow: hidden; z-index: 10; padding: 0; margin: 0; text-align: center;' title='{tooltip}'><div style='line-height: 1.2; pointer-events: none; width: 100%; padding: 0 4px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;'>{base_text}</div></a>"
...
# 👇 --- ΔΙΑΒΑΣΜΑ ΚΑΙ ΑΠΟΚΩΔΙΚΟΠΟΙΗΣΗ ΤΟΥ ΣΩΣΤΟΥ ΚΛΙΚ --- 👇
if clicked_id:
    try:
        # Επαναφέρουμε τα '=' που κόψαμε πιο πάνω, για να μπορεί να διαβαστεί το ID!
        padding = "=" * ((4 - len(clicked_id) % 4) % 4)
        decoded_key = base64.urlsafe_b64decode((clicked_id + padding).encode('utf-8')).decode('utf-8')
