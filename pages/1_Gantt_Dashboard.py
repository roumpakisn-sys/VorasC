...
if export_data:
    # Performance: χτίζουμε τα export payloads μόνο όταν αλλάζει το γράφημα/δεδομένα.
    export_params = dict(current_gantt_params)
    if (
        st.session_state.get('last_export_payload_params') != export_params
        or 'cached_export_excel_bytes' not in st.session_state
        or 'cached_export_html_bytes' not in st.session_state
        or st.session_state.get('cached_export_excel_bytes') is None
        or st.session_state.get('cached_export_html_bytes') is None
    ):
        ...
        st.session_state.cached_export_excel_bytes = excel_buffer.getvalue()
        st.session_state.cached_export_html_bytes = fig.to_html(full_html=True, include_plotlyjs="cdn").encode("utf-8")
        st.session_state.last_export_payload_params = export_params
    ...
    with col_btn_excel:
        st.download_button(
            label="📥 Εξαγωγή (Excel)",
            data=st.session_state.cached_export_excel_bytes,
    ...
    with col_btn_html:
        st.download_button(
            label="🌐 Εξαγωγή (HTML)",
            data=st.session_state.cached_export_html_bytes,
            ...
            if not wk_groups:
                ...
                if selected_key != "":
                    ...
                    with st.form("quick_edit"):
                        ...
                        # Κρατάμε μόνο τους πραγματικούς εργαζόμενους της μπάρας.
                        # Δεν κάνουμε πλέον auto-προσθήκη από παλιά notes σύγκρουσης,
                        # γιατί αυτό δημιουργεί ψευδείς διπλοκρατήσεις στο save.
