...
def go_to_today(): st.session_state.view_week_date = get_local_today()

# --- VIEW: DASHBOARD (GANTT) ---
st.title("📊 Εβδομαδιαίο Χρονοδιάγραμμα Πόρων")

# 1. Εμφάνιση του πάνω μενού μέσω του ui_dashboard
...
# --- ΚΑΤΑΣΚΕΥΗ GANTT CHART (ΟΠΩΣ ΠΡΙΝ) ---
...
# 2. Εμφάνιση ενότητας Εξαγωγής μέσω του ui_dashboard
ui_dashboard.render_export_section(export_data, start_of_week)

# --- 3. ΦΟΡΜΕΣ ΠΡΟΣΘΗΚΗΣ ΚΑΙ ΕΠΕΞΕΡΓΑΣΙΑΣ ΜΕΣΩ ΤΟΥ UI_DASHBOARD ---
if not presentation_mode:
    ...
    if is_full_admin:
        ...
        with col_add:
            qa_res = ui_dashboard.render_quick_add_form(selected_date, active_employee_ids)
            if qa_res:
                add_date = qa_res["add_date"]
                duration_days = qa_res["duration_days"]
                proj_choice = qa_res["proj_choice"]
                custom_proj_name = qa_res["custom_proj_name"]
                emp_choices = qa_res["emp_choices"]
                color_choice = qa_res["color_choice"]
                add_notes = qa_res["add_notes"]
                use_arr = qa_res["use_arr"]
                t_arrival = qa_res["t_arrival"]
                t_start = qa_res["t_start"]
                t_end = qa_res["t_end"]
                ...
                if str_start >= str_end:
                    ...
                    # ΒΡΟΧΟΣ (ΛΟΥΠΑ) ΓΙΑ ΤΙΣ ΣΥΝΕΧΟΜΕΝΕΣ ΗΜΕΡΕΣ
        ...
        with col_edit:
            edit_res = ui_dashboard.render_edit_form(wk_groups, clicked_key, active_employee_ids)
            if edit_res:
                action = edit_res["action"]
                target_group = edit_res["target_group"]
                
                if action == "move":
                    delta_days = edit_res['delta_days']
                    delta_hours = edit_res['delta_hours']
                    has_error = False
                    ...
                    edit_date = edit_res["edit_date"]
                    edit_proj = edit_res["edit_proj"]
                    edit_custom_proj_name = edit_res["edit_custom_proj_name"]
                    edit_emps = edit_res["edit_emps"]
                    edit_color = edit_res["edit_color"]
                    edit_notes = edit_res["edit_notes"]
                    use_arr_edit = edit_res["use_arr_edit"]
                    new_t_arrival = edit_res["new_t_arrival"]
                    new_t_start = edit_res["new_t_start"]
                    new_t_end = edit_res["new_t_end"]
                    e_is_cancelled = edit_res["e_is_cancelled"]
                    e_cancel_reason = edit_res["e_cancel_reason"]
